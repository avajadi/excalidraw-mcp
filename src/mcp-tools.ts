/**
 * The excalidraw MCP tool set, as an `McpServer` factory shared by both transports:
 * `index.ts` connects one over stdio (spawned per Claude session, or run standalone
 * in file-mode), and `relay.ts` connects a fresh one per HTTP request to serve MCP
 * directly at `/mcp` (see StreamableHTTPServerTransport's stateless mode — every
 * tool call here is already self-contained, fetching scene state fresh per call,
 * so there's no session state to lose by not reusing a server instance).
 */
...
/**
 * Build an MCP server exposing the excalidraw tools. `relayUrl` set (e.g.
 * `http://localhost:3030`) = scenes live in the long-lived relay for live viewing
 * and co-editing; unset = file-only mode (each scene is a file).
 */
export function createServer(relayUrl: string | undefined): McpServer {
  const RELAY_URL = relayUrl?.replace(/\/+$/, "");
